...
public class FolderService {
    ...
    public FolderEntity createRootIfNotExists() {
        FolderEntity root = folderRepository.findByFolderName("root");

        if (root == null) {
            root = FolderEntity.builder()
                    .folderName("root")
                    .path(Path.of("app", "storage-files", "root").toString())
                    .build();

            fileUtils.createDirectory(Path.of("app", "storage-files"), root.getFolderName());
    ...
    private ArrayList<FolderEntity> createAndAddNodes() {
        ArrayList<FolderEntity> lastCreatedFolders = folderRepository.findByIsLast(true)
                .orElseThrow(() -> new IOException("Something went wrong."));
    ...
    private List<FolderEntity> createNewFolder(FolderEntity parentFolder) {
        ...
        for (byte i = 0; i < 2; i++) {
            ...
            fileUtils.createDirectory(parentPath, folderName);
